...
_DEFAULT_CRF_SKILLS_PATH = os.path.join(_BASE_DIR, "models", "crf_skills_v2.pkl")
...
def analyze_resume_for_role(
    ...
    preprocessor  = ResumePreprocessor(
        crf_model_path=resolved_crf_path,
        crf_skills_model_path=_DEFAULT_CRF_SKILLS_PATH,
    )
    ...
    # ── Step 3b — extract real-world skill sets from the profile ────
    # These override the TF-IDF job_result skills in the matching engine
    # so scoring is driven by evidence-based profiles, not free text.
    required_skills_set  = {s.lower() for s in job_profile.get("required_skills",  [])}
    preferred_skills_set = {s.lower() for s in job_profile.get("preferred_skills", [])}
    ...
    fit_analysis = analyze_resume_job_fit(
        ...
        preferred_skills=preferred_skills_set,
    )
